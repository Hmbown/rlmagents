...
rlmagents stores the full context in a REPL namespace (default variable: `ctx`).
The root LLM can write Python code to inspect and process the context via helper
functions.
...
Do not expose rlmagents code-execution mode to untrusted users without stronger
...
from rlmagents.repl import helpers as _helpers
...
def _execute_with_timeout(
    ...
    if can_use_signal:
        ...
        def _timeout_handler(signum: int, frame: object) -> None:
            raise ExecutionTimeout(f"Code execution exceeded {timeout_seconds:.1f}s timeout")
...
def _compile_with_last_expr(source: str) -> tuple[CodeType, CodeType | None]:
    ...
    if tree.body:
        ...
        if isinstance(last_stmt, ast.Expr):
            ...
            exec_code = compile(tree, filename="<rlmagents_repl>", mode="exec")
            eval_code = compile(expr, filename="<rlmagents_repl_expr>", mode="eval")
        ...
        if isinstance(node, ast.Subscript):
            if isinstance(node.slice, ast.Constant):
                if isinstance(node.slice.value, str) and node.slice.value.startswith("__"):
                    raise SecurityError(f"Subscript access to '{node.slice.value}' is not allowed")
...
class REPLEnvironment:
    ...
    def __init__(
        ...
        self._sub_query_fn: Callable[[str, str | None], Any] | None = None

    def set_loop(self, loop: asyncio.AbstractEventLoop | None) -> None:
        """Set/replace the event loop used to bridge async calls."""
        self._loop = loop

    def inject_sub_query(self, fn: Callable[[str, str | None], Any]) -> None:
        """Inject sub_query(prompt, context_slice=None) into the REPL namespace.

        Also injects ``llm_query`` as an alias, matching the naming
        convention from the RLM paper.
        """
        self._sub_query_fn = fn
        bridged = self._sync_bridge(fn)
        self._namespace["sub_query"] = bridged
        self._namespace["llm_query"] = bridged
    ...
    def _sync_bridge(self, fn: Callable[..., object | Awaitable[object]]) -> Callable[..., object]:
        ...
        def _wrapped(*args: object, **kwargs: object) -> object:
            ...
            if threading.current_thread() is threading.main_thread() and self._loop.is_running():
                raise RuntimeError(
                    "sub_query called from the event loop thread. "
                    "Use execute_async() or run in a thread."
                )
    ...
    def execute(self, code: str) -> ExecutionResult:
        ...
        still allowing sub_query, run it in a worker thread.
        """
        ...
        try:
            ...
            if ret is not None:
                if isinstance(ret, str):
                    return_value, return_truncated = _truncate_output_text(
                        ret, self.config.max_output_chars
                    )
                else:
